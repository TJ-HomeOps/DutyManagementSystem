...
export class AuthService {
  ...
  async getStatus(): Promise<{ enabled: boolean }> {
    ...
    return { enabled: settings.lockEnabled };
  }
  ...
  // There is no separate "change password" flow because the Settings page
  // itself is only reachable once already authenticated, so re-flicking the
  // toggle is the change-password flow.
